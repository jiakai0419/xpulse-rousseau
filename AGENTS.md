...
- Run `npm run doctor` before changing dependencies, browser tooling, API integration, or server startup behavior.
...
- Add tests for new domain logic. For UI-only changes, run `npm run browser:smoke` when browser tooling is available.
- Prefer `npm run dev`, `npm run stop`, and `npm run smoke` over ad hoc server commands.
