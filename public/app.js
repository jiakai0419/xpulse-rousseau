...
function receiptFromRecords(title, records = []) {
  if (!records.length) {
    return undefined;
  }
...
function renderUsageDetails(receipt) {
...
function progressText(progress) {
  const label = progressLabels[progress.stage] ?? progress.label ?? "Working";
...
function progressDetail(progress, totals) {
...
function renderServerProgress(progress, startedAt) {
  ...
  statusNode.textContent = progressLabels[progress.stage] ?? progress.label ?? "Working";
...
function variantDimensions(url) {
...
function variantFitsInline(variant, size) {
...
function fullSizeMediaUrl(media) {
...
function mediaAspectRatioValue(media) {
  if (media.width && media.height) {
...
function mediaRatioNumber(media) {
  if (media.width && media.height) {
...
function mediaGalleryAspectRatio(media, count) {
  if (count === 2) {
...
function singleMediaWidth(media) {
  if (!media.width || !media.height) {
    return "100%";
  }
