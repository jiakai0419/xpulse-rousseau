...
| Scoring preserves quote context nested inside a reposted source | `tests/unit/scoring.test.ts` | Sanitized real X-derived structure plus constructed scores | Covered |
...
| Server commit saves a completed run and mutates Seen Ledger / Timeline Cursor only for Online runs; later failures restore all three stores | `tests/unit/commitRefreshRun.test.ts` | Constructed run objects plus real temporary file repositories | Covered |
...
| X usage counts failed pagination/lookup and failed freshness attempts before fallback | `tests/unit/xClient.test.ts`, `tests/unit/xEnrichment.test.ts`, `tests/unit/pipelineProgress.test.ts` | Constructed X responses/errors | Covered |
| Link preview HTML and cached preview images reject private/special IPs, revalidate every DNS-pinned redirect hop, and never expose metadata image URLs directly to the browser | `tests/unit/linkPreview.test.ts`, `tests/unit/readerLinkRules.test.ts`, `tests/unit/readerPostLinks.test.ts`, `npm run test:server-entry` | Local resolver/request harness, constructed legacy cache shape, and isolated server | Covered |
| Local JSON state is private, atomic, crash-safe, and serialized across concurrent updates | `tests/unit/privateJsonFile.test.ts`, `tests/unit/linkPreview.test.ts` | Temporary real files | Covered |
| Empty smoke credentials cannot be restored from `.env` | `tests/unit/serverEnv.test.ts`, `npm run test:server-entry` | Temporary dotenv file plus isolated server | Covered |
| Reader startup recovers running/completed/failed jobs without an old latest response winning | `tests/unit/readerRecovery.test.ts` | Constructed job/run states | Covered |
| Refresh job memory is bounded and response jobs omit full traces | `tests/unit/refreshJobs.test.ts` | Constructed jobs/runs | Covered |
| Reader identity and default source match the actual time-stable OAuth/manual X credential choice, including refresh-window drift | `tests/unit/xSourceCredentials.test.ts`, `tests/unit/readerSourceStatus.test.ts` | Constructed credential/status states and failed OAuth refresh | Covered |
| Progress is monotonic across stages and failures remain visible | `tests/unit/readerStatus.test.ts`, `tests/unit/pipelineProgress.test.ts` | Constructed progress/jobs | Covered |
| Modal focus, quote semantics, inline video controls/loading, and explicit Signal scale stay accessible | `tests/unit/readerPostMedia.test.ts`, `tests/unit/readerPostQuote.test.ts`, `tests/unit/readerActions.test.ts`, `npm run test:smoke-ui` | Constructed rendering states plus saved X-derived run | Covered |
...
| Generated-evidence pruning is bounded to known timestamped families and preserves baselines/unknown data | `tests/unit/dataPrune.test.ts`, `npm run data:prune` | Constructed paths plus owner-reviewed local dry run | Stewardship guard |
| Shared CI type-checks and executes the real HTTP entry without provider credentials | `npm run typecheck`, `npm run test:server-entry`, `.github/workflows/ci.yml` | Application source plus isolated empty server | Shared CI guard |
| Common credential-shaped values are rejected before CI passes | `tests/unit/secretScan.test.ts`, `npm run security:secrets` | Constructed secret shapes plus worktree files | Shared CI guard |
| Only one process can write product state; dead owners are safely reclaimed and normal SIGTERM drains active jobs/requests before removing the lock | `tests/unit/serverStateLock.test.ts`, `tests/unit/serverActivityTracker.test.ts`, `npm run test:server-entry` | Temporary real lock files, constructed in-flight request boundaries, and isolated server | Covered |
| Local HTTP rejects DNS-rebinding Hosts, cross-site mutations/proxy embeds, non-JSON Pulse bodies, and oversized bodies | `tests/unit/requestSecurity.test.ts`, `npm run test:server-entry` | Constructed headers/bodies plus isolated server | Covered |
...
| Historical display inventories use only the newest eligible live X runs with trace input posts | `tests/unit/displayInventoryHistoryRuns.test.ts`, `npm run x-display:collect-local-renderings` | Constructed run metadata plus saved X-derived live runs | Covered |
...
- The server job commit path is extracted to `src/services/pipeline/commitRefreshRun.ts`, uses repository checkpoints plus compensating rollback, and is directly covered by unit and real-file tests. Future server refactors should keep that coordinator out of HTTP route code.
