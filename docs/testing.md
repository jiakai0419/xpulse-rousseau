...
`npm run test:smoke-api` and `npm run test:smoke-ui` start temporary local servers and force `TIMELINE_SOURCE=replay`. Before starting the server, each script copies the latest saved live X run from `.data/runs.json` into a smoke-specific run store under `.data/`, so verification does not reorder the user's real local reading history. The temporary run store is removed when the script exits. For UI fidelity checks against a known historical live run, pass `BROWSER_SMOKE_RUN_ID=<run-id>` to `npm run test:smoke-ui`.

Smoke replay does not call X or OpenAI. If `.data/runs.json` has no saved live X run, API/UI smoke fails and the fix is to run one live X refresh first.
...
`npm run refactor:check-baseline` is the local pre-refactor baseline. It runs `env:check`, unit tests, API smoke, UI smoke, and `x-display:test-replay-rendering` in order, stopping at the first failure. It deliberately does not run Original X evidence capture or `x-display:test-fresh-pulse-rendering`, because those may require reachable Original X pages or real X/OpenAI spend.
...
- **Local data stewardship:** `npm run data:inventory` explains what is in `.data` before cleanup, pruning, or evidence-heavy refactoring. It is a read-only map, not a cleanup command.
...
GitHub Actions CI is intentionally narrower than the local refactor baseline. CI runs `npm ci`, `npm run env:check`, `npm run test:unit`, and `npm run test:coverage` on push and pull request. It does not run `.data`-dependent replay/display tests or real X/OpenAI audits. Treat CI as the shared logic gate, and treat `npm run refactor:check-baseline` plus X display/fresh Pulse validation as the local product-fidelity gate.
